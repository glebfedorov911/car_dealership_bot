...
async def check_auto(message: Message) -> None:
    if message.text == 'Посмотреть автомобили🚗':
        ...
        auto = Card(brand=-1, model=-1, call="car", query=query_auto)
...
async def answer_to_callback_car(query: CallbackQuery) -> None:
    ...
    try:
        ...
        if action[1] == 'stay':
            if action[-1] == '-1':
                query_model = db.select_data(name_table='specifications', data=['*'], relate='', where=True, where_data={'brand_id':action[2]})
                model = Card(brand=action[2], model=-1, call='model', query=query_model)
                await query.message.answer('Выберите модель автомобиля', reply_markup=model.show())
            else:
                query_model = db.select_data(name_table='specifications', data=['*'], relate='AND', where=True, where_data={'brand_id':action[2], 'model':action[3]})
